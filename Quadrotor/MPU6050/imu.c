...
//		MPU6050_Raw_Data.Mag_Z = values[8];
		lastgyro[0]=Sys.YawS;
		lastgyro[1]=Sys.PitchS;
		lastgyro[2]=Sys.RollS;
		Sys.YawS=-values[5];
		Sys.PitchS=-values[4];
		Sys.RollS=values[3];
		Sys.YawA=Sys.YawS-lastgyro[0];
		Sys.PitchA=Sys.PitchS-lastgyro[1];
		Sys.RollA=Sys.RollS-lastgyro[2];
}
...
void Q2Euler(volatile float * q,volatile float * angles){
    ...
    angles[1] = -asin(-2 * q[1] * q[3] + 2 * q[0] * q[2])* 180/M_PI; // pitch    -pi/2    --- pi/2 
    angles[2] = atan2(2 * q[2] * q[3] + 2 * q[0] * q[1], -2 * q[1] * q[1] - 2 * q[2] * q[2] + 1)* 180/M_PI; // roll       -pi-----pi  
		Sys.Yaw=angle[0]-Par.YO/100.0f;
		Sys.Pitch=angle[1]-Par.PO/100.0f;
		Sys.Roll=angle[2]-Par.RO/100.0f;
}
