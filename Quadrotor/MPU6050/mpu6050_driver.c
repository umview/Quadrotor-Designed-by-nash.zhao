...
//		HMC58X3_ReadData(&(mpu_buf[14]));  //14-19为陀螺仪数据
		...
		*gx  =MPU6050_FIFO[3][10] - Info.GyroXOffset;
		*gy = MPU6050_FIFO[4][10] - Info.GyroYOffset;
		*gz = MPU6050_FIFO[5][10] - Info.GyroZOffset;
//		*ax  =MPU6050_Lastax;
//		*ay  =MPU6050_Lastay;
//		*az = MPU6050_Lastaz;
//		*gx  =MPU6050_Lastgx - Info.GyroXOffset;
//		*gy = MPU6050_Lastgy - Info.GyroYOffset;
//		*gz = MPU6050_Lastgz - Info.GyroZOffset;
//	} 
//	else
//	{       //读取上一次的值
...
//		*gx = MPU6050_FIFO[3][10]-Info.GyroXOffset;//=MPU6050_FIFO[3][10];
//		*gy = MPU6050_FIFO[4][10]-Info.GyroYOffset;//=MPU6050_FIFO[4][10];
//		*gz = MPU6050_FIFO[5][10]-Info.GyroZOffset;//=MPU6050_FIFO[5][10];
//	}
}
...
void HMC58X3_mgetValues(volatile float *arry) 
{
    ...
    arry[0]= HMC5883_lastx=((float)(xr - Info.MagXOffset)) * Info.MagXScale;
    arry[1]= HMC5883_lasty=((float)(yr - Info.MagYOffset)) * Info.MagYScale;
    arry[2]= HMC5883_lastz=((float)(zr - Info.MagZOffset)) * Info.MagZScale;
}
void getOffset(){
//	int sum[3]={0,0,0};
//	uint16_t i=0;
...
//		i++;
//		MPU6050_ReadData(MPU6050_DEVICE_ADDRESS,MPU6050_DATA_START,mpu_buf,14);  //
//		sum[0]+=(((int16_t)mpu_buf[8]) << 8) | mpu_buf[9];
//		sum[1]+=(((int16_t)mpu_buf[10]) << 8) | mpu_buf[11];
//		sum[2]+=(((int16_t)mpu_buf[12]) << 8) | mpu_buf[13];
//		delay_ms(5);
//		if(i>=200){
//			Info.GyroXOffset=(int16_t)(sum[0]/200.0f);
//			Info.GyroYOffset=(int16_t)(sum[1]/200.0f);
//			Info.GyroZOffset=(int16_t)(sum[2]/200.0f);
//			break;
//		}
//	}
		Info.GyroXOffset=-69;
